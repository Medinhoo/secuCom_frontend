...
interface CompanyRecentActivitySectionProps {
  companyId: string;
}

export const CompanyRecentActivitySection: React.FC<CompanyRecentActivitySectionProps> = ({ companyId }) => {
  const { dimonas: inProgressDimonas, isLoading: inProgressLoading } = useDimonasInProgress();
  const { dimonas: toConfirmDimonas, isLoading: toConfirmLoading } = useDimonasToConfirm();
  ...
  const isLoading = inProgressLoading || toConfirmLoading || notificationsLoading;

  // Filter dimonas for this company
  const companyInProgressDimonas = inProgressDimonas.filter(dimona => dimona.companyId === companyId);
  const companyToConfirmDimonas = toConfirmDimonas.filter(dimona => dimona.companyId === companyId);

  // Combine and sort recent dimonas by entry date
  const allRecentDimonas = [...companyInProgressDimonas, ...companyToConfirmDimonas]
  ...
  useEffect(() => {
    const fetchCollaboratorNames = async () => {
      ...
      for (const dimona of allRecentDimonas) {
        if (!names[dimona.collaboratorId]) {
          try {
            const collaborator = await collaboratorService.getCollaboratorById(dimona.collaboratorId);
            names[dimona.collaboratorId] = `${collaborator.firstName} ${collaborator.lastName}`;
          } catch (error) {
            names[dimona.collaboratorId] = 'Nom non disponible';
          }
        }
      }
              ...
              <div className="flex items-center gap-2 flex-1 min-w-0">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="text-xs text-gray-500 truncate">
                    {collaboratorNames[dimona.collaboratorId] || 'Chargement...'}
                  </div>
                </div>
              </div>
              <div className="flex flex-col items-end gap-1 flex-shrink-0">
